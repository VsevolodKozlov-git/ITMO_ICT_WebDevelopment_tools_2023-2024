...
def main():
    with get_session_func() as session:
        ...
        # Create projects
        project_university = Project(
            title="University", description="Project related to university tasks"
        )
        project_soft_skills = Project(
            title="Soft skills", description="Project related to improving soft skills"
        )
        ...
        links = [
            ProjectUserLink(
                user_id=users[0].id, project_id=project_university.id, role=Role.admin
            ),
            ProjectUserLink(
                user_id=users[1].id, project_id=project_university.id, role=Role.viewer
            ),
            ProjectUserLink(
                user_id=users[2].id, project_id=project_university.id, role=Role.viewer
            ),
            ProjectUserLink(
                user_id=users[3].id, project_id=project_soft_skills.id, role=Role.admin
            ),
            ProjectUserLink(
                user_id=users[4].id, project_id=project_soft_skills.id, role=Role.viewer
            ),
        ]
        ...
        tasks_university = [
            Task(
                ...
                description="Work on the research paper.",
                priority=Priority.high,
                approximate_time=datetime.now().time(),
                category_id=categories_university[0].id,
            ),
            Task(
                title="Study Group",
                ...
                approximate_time=datetime.now().time(),
                category_id=categories_university[1].id,
            ),
        ]
        ...
        tasks_soft_skills = [
            Task(
                ...
                priority=Priority.medium,
                approximate_time=datetime.now().time(),
                category_id=categories_soft_skills[0].id,
            ),
            Task(
                title="Team Collaboration",
                description="Work on team collaboration techniques.",
                priority=Priority.high,
                approximate_time=datetime.now().time(),
                category_id=categories_soft_skills[1].id,
            ),
        ]
